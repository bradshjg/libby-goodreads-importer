...
import {GoodreadsExportItem, LibbyImportItem} from './types'
...
const goodreadsData = (isbn: string, shelves: string, dateAdded: (Date | undefined), dateRead: (Date | undefined)): GoodreadsExportItem => {
...
test('shelve borrowed books with empty details as read', () => {
  ...
  expect(transformCSV([libbyData('some-isbn', 'Borrowed', '', today, )])).toEqual(
    [goodreadsData('some-isbn', 'read', undefined, today)]
  )
})
